...
动态主视觉编辑会沿 selector、顶层 DOM 变量及有界派生变量关联实际渲染函数，避免只暴露空挂载节点。局部模型返回目标类型、重复目标、目标 ID 或源哈希等结构错误时会携带错误码严格重试一次；JavaScript 驱动的主视觉补丁重试后仍未通过安全校验时，在完整输出预算充足的前提下允许回退完整编辑，纯 CSS 修改仍保持局部失败阻断。请求级 LangSmith Trace 输出包含 SSE 业务 `outcome`、`completed` 和 `error_code`，用于区分传输成功与业务失败。
...
8. 生成、编辑和模型修复的候选结果都会重新经过同一个服务端布局装配器。`phase=edit_html` 会先执行 `extract_business_html`，按 CSS 声明、CSS 规则、`<style>`、完整业务 HTML 的顺序控制修改范围；局部补丁被拒绝、截断或无法应用时，默认基于当前业务 HTML 和用户指令重新生成完整业务 HTML，只有显式关闭 `AETHERVIZ_EDIT_FULL_HTML_FALLBACK_ENABLED` 才保留原页面并返回局部编辑错误。运行时目标不再只按 `play/loop/animate` 等函数名选择，而是优先从控件事件绑定和 `AetherVizRuntime` 动作出发，沿函数调用、共享控制器和顶层共享状态依赖构建最多 5 个函数的闭合切片；固定函数名仅作为无法找到语义入口时的降级证据。补丁若只修改主控制链之外的 fallback、引入新的未声明标识符，或仍保留动画控制器契约错误，会回滚并进入完整业务 HTML 编辑。完整编辑不能改变布局外壳，只能修改数学内容、业务交互与槽位优先级。编辑候选会继续执行确定性视觉/动画质量收尾，但不会为 warning 额外发起一次完整模型重写。结果仍生成新 HTML 分支，不覆盖旧 HTML。
