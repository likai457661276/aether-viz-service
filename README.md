...
8. 生成、编辑和模型修复的候选结果都会重新经过同一个服务端布局装配器。`phase=edit_html` 会先执行 `extract_business_html`，按 CSS 声明、CSS 规则、`<style>`、完整业务 HTML 的顺序控制修改范围；局部样式失败会保留原页面而不是无条件扩大编辑范围。运行时问题优先走小输出补丁，完整编辑只处理业务 HTML；它不能改变布局外壳，只能修改数学内容、业务交互与槽位优先级。编辑候选会继续执行确定性视觉/动画质量收尾，但不会为 warning 额外发起一次完整模型重写。结果仍生成新 HTML 分支，不覆盖旧 HTML。
