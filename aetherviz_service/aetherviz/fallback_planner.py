...
- 数学主题固定使用 HTML + CSS + SVG + KaTeX + 原生 JavaScript/CSS 动画。
...
- mode：generic_svg / math_svg_katex_css
- title：页面标题
- goal：一句话教学目标
...
def select_generation_mode(topic: str) -> str:
    return "math_svg_katex_css" if detect_subject(topic) == "math" else "generic_svg"
...
def normalize_plan(raw_plan: dict | None, topic: str, primary_color: str = DEFAULT_PRIMARY_COLOR) -> dict:
    ...
    if mode not in {"generic_svg", "math_svg_katex_css", "math_svg_katex_gsap"}:
        mode = "math_svg_katex_css" if subject == "math" else "generic_svg"
    if subject == "math":
        mode = "math_svg_katex_css"

    formulas = _string_list(raw.get("formulas"), fallback["formulas"], max_items=4, max_len=80)
    if mode == "math_svg_katex_css" and not formulas:
    ...
    if mode == "math_svg_katex_css":
        return {
            ...
            "validation_points": [
                "包含 #math-svg",
                "使用 KaTeX 渲染公式",
                "使用 CSS 动画或 requestAnimationFrame 管理动画",
