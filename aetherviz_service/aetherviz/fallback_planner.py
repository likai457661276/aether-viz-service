...
4. **动画运行时（animation_runtime）**决定动画编排职责：
   ...
   - Canvas 粒子、波动、碰撞等高频场景优先 native；如选 gsap_timeline，GSAP 只驱动全局 progress 或阶段，不逐对象驱动粒子。

5. **number_design** 必须给出默认数值和设计理由。默认数值应通俗、可心算、符合教学目标，避免随机长小数。

6. **stage_layout** 描述首屏舞台编排：目标导航、主舞台、公式/结论区、控制条如何摆放，必须避免小图挤在角落。
...
10. **controls** 只保留真正影响核心原理理解的控件，不超过 4 个。
...
- controls：2~4 个控件，每个包含 id、label、type（slider/button/speed）
...
输出 JSON 示例：
{
  ...
  "stage_layout": "顶部用目标胶囊提示学习路径，中间大舞台展示可拖拽图形，底部固定控制条和公式结论区。",
  "storyboard": [
...
def _default_plan(topic: str, primary_color: str = DEFAULT_PRIMARY_COLOR) -> dict:
    ...
    if subject == "math":
        ...
        return {
            ...
            "stage_layout": "顶部展示学习目标导航，中间用大比例 SVG 舞台呈现核心图形，底部集中放置参数滑块、播放按钮和公式结论。",
            "storyboard": [
    ...
    if subject in ("chemistry", "biology"):
        ...
        return {
            ...
            "stage_layout": "顶部用阶段导航说明流程，中间用 DOM/SVG 大舞台展示过程节点，右侧或底部显示当前阶段解释和关键变化。",
            "storyboard": [
    ...
    return {
        ...
        "stage_layout": "顶部展示学习目标，中央保留大面积动画舞台，底部控制条负责播放、重置、速度和主要参数，公式或结论固定在舞台下方。",
        "storyboard": [
        ...
        "controls": [
            {"id": "progress-slider", "label": "进度", "type": "slider"},
            {"id": "speed-control", "label": "速度", "type": "speed"},
...
def _default_number_design(topic: str, subject: str) -> dict:
    ...
    return {
        "default_values": ["进度 = 0%", "速度 = 1x", "步骤 = 3"],
        "reason": "使用标准进度、默认速度和三段式步骤，便于学生从初始状态逐步观察到结论。",
    }
