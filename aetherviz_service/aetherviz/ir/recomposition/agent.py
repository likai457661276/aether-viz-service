...
frames 必须与 stage_requirements 一一对应，使用 3~5 个静态对象 {{"stage_id":"计划阶段 id","at":与计划阶段一致的数值,"caption":"教学文本","formula":"公式或关系","step":非负整数}}；stage_id 与 at 必须逐项复制计划，不得自创时间线。每个中间 stage_requirements 都必须在图元 keyframes 中出现同一 at；至少达到该阶段 min_piece_ratio 的图元要在该时刻形成区别于 source、target 且偏离首尾直接线性插值的独立几何状态，相对任一参照的偏离至少满足：平移≥12px，或旋转≥8°，或 |Δscale|≥0.04，或 |Δopacity|≥0.06。keyframes 首尾 at 必须为 0/1，总数必须等于 frames/stage_requirements；不得输出只有文字说明、没有对应几何关键状态的中间阶段，也不得依赖服务端事后补帧。
几何属性、source/target 和教学解释必须描述同一切分重排；拓扑变量只用于 repeat.count，普通几何变量只改变坐标或尺度。
同一 repeat 生成全等拼片时，id 严禁使用固定字符串，必须依赖 repeat.index 保证展开后唯一，例如 {{"op":"concat","args":["piece-",{{"local":"i"}}]}}；attrs 必须定义统一的局部坐标几何；每片在源状态和目标状态的朝向必须只由 source/target.rotation 表达。禁止把 repeat 索引对应的绝对朝向同时编码进 attrs（例如 sector_path 的起止角）和 transform.rotation，避免重复旋转。目标声称对齐、合并或拼成整体时，必须在世界坐标下让拼片边界真实接触，且满足 target_assembly 的连通、重叠率和整体形状阈值。
...
def _stream_generate_recomposition_html_impl(
    ...
    try:
        ...
        ir_repair_attempts = 1
        try:
            source = _repair_scene_source(topic, plan, exc.raw_text, exc.report)
        except HtmlGenerationError as repair_exc:
            repair_exc.diagnostics.setdefault("ir_repair_attempts", ir_repair_attempts)
            raise
            ...
            raise HtmlGenerationError(
                ...
                diagnostics={"ir_repair_attempts": ir_repair_attempts},
            ) from repair_exc
        degraded = True
    ...
    yield HtmlStreamResult(
        ...
        degraded=degraded,
        truncated=False,
        ir_repair_attempts=ir_repair_attempts,
    )
...
def _repair_scene_source(
    ...
    prompt = (
        ...
        "先按 stage_alignment.frames_must_copy 逐项复制 stage_id/at；若报告含 missing_intermediate_geometry_stage，"
        "必须为失败中间阶段补齐同 at 的非线性 keyframe，覆盖至少 min_piece_ratio 比例图元，"
        "并同时显著偏离 source、target 与直线插值，达到 evidence_thresholds；禁止只改 frames 文字。"
        "同一 repeat 的 id 必须依赖 repeat.index，例如 "
        '{"op":"concat","args":["piece-",{"local":"i"}]}，严禁使用固定字符串 id；'
        "全等拼片使用统一局部几何，只通过 source/target.rotation 表达各阶段朝向；"
...
def _build_scene_prompt(topic: str, plan: dict[str, Any]) -> str:
    ...
    return (
        ...
        "生成前必须先按 stage_alignment.frames_must_copy 逐项复制 frames 的 stage_id/at；"
        "每个中间阶段为至少 min_piece_ratio 比例的图元写入同 at 的非线性几何 keyframe，"
        "相对 source、target 与直线插值至少达到 evidence_thresholds（平移像素、旋转角度、scale 或 opacity）；"
        "不得把中间阶段只写成教学文字，也不得指望服务端确定性补帧。\n"
...
def _stage_alignment_checklist(plan: dict[str, Any]) -> dict[str, Any]:
    """Expose plan stage_requirements as an explicit candidate-generation contract."""
    ...
    frames_must_copy = [
        {
            ...
            "min_piece_ratio": stage.get("min_piece_ratio"),
            "intent": str(stage.get("intent") or "")[:120],
        }
        for stage in stages
    ]
    intermediate = [
        {
            "id": item["id"],
            "at": item["at"],
            "min_piece_ratio": item["min_piece_ratio"],
            "intent": item["intent"],
        }
        for item in frames_must_copy
        if item["role"] == "intermediate"
    ]
    return {
        "frames_must_copy": frames_must_copy,
        "intermediate_stages": intermediate,
        "evidence_thresholds": dict(INTERMEDIATE_EVIDENCE_THRESHOLDS),
        "rules": [
            "frames 数量、顺序、stage_id、at 必须与 frames_must_copy 完全一致",
            "每个 intermediate_stages 项都必须在足够比例图元的 keyframes 中出现同一 at",
            "该时刻变换须同时显著偏离 source、target，以及 source→target 的直接线性插值",
            "仅有 frames 文字说明、keyframes 缺失或仍走直线插值，一律视为候选失败",
        ],
    }
        ...
        "errors": [
            {
                ...
                "hard_failures": item["hard_failures"],
                "score": item["score"],
                "schema_diagnostics": _compact_schema_diagnostics(
                    item.get("details", {}).get("schema", {})
                ),
...
def _compact_schema_diagnostics(report: object) -> list[dict[str, Any]]:
    if not isinstance(report, dict):
        return []
    allowed_keys = ("type", "message", "path", "state", "index", "name")
    return [
