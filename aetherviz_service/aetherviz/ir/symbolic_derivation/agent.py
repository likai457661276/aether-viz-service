...
def stream_generate_symbolic_derivation_html(
    ...
    if not ranking["ok"]:
        ...
        try:
            ...
            ranking = {"ok": False}
    if not ranking["ok"]:
        logger.warning("symbolic derivation IR invalid; falling back to direct HTML")
        for item in stream_generate_html(topic, plan):
            yield (
                replace(item, degraded=True, generation_fallback="symbolic_derivation_ir_invalid")
