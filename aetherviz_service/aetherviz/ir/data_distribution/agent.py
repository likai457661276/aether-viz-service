...
首版不生成随机样本、不累计随机试验，也不计算连续概率密度或曲线下面积；遇到这些目标应由直接 HTML 降级处理。所有学生可见文字使用简体中文。IR 不超过 {DATA_DISTRIBUTION_IR_MAX_CHARS} 字符。"""
...
def stream_generate_data_distribution_html(
    ...
    if not ranking["ok"]:
        ...
        try:
            ...
            ranking = {"ok": False}
    if not ranking["ok"]:
        logger.warning("data distribution IR invalid; falling back to direct HTML")
        for item in stream_generate_html(topic, plan):
            yield (
                replace(item, degraded=True, generation_fallback="data_distribution_ir_invalid")
