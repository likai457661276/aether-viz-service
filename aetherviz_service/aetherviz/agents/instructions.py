...
10. 互动验收（系统提示已覆盖的通用规则不再重复，以下为本次任务的补充要点）
- #aetherviz-stage 内主 SVG/Canvas 居中：SVG 使用 preserveAspectRatio="xMidYMid meet"，主舞台补充 data-region="stage"。
- 完整分镜/教学流程列表需覆盖 teaching_flow 条目；控件必须绑定真实功能。
- 不输出页脚署名、品牌署名或生成来源文案。
