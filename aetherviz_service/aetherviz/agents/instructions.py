...
- range 只声明标准 input 的 id、data-var、min、max、step、value 和无障碍标签；不要自绘轨道或进度。服务端 range-v1 会根据实时 value 自动计算进度并覆盖跨浏览器样式。
...
- 如果 `window.gsap` 不可用，必须保留可运行的 native fallback；可优先使用服务端提供的 `window.AetherVizAnimationController.create({{duration, update}})`，确保 GSAP 与 fallback 共用同一 deriveView/applyView 路径。
