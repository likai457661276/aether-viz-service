...
- 模型只负责可提取内容：#aetherviz-stage 主视觉、[data-region="controls"] 业务控件、[data-region="caption"] 当前说明、[data-region="formula"] 公式/结论、[data-region="teaching-flow"] 教学流程、编辑基线已有的 [data-shell-content-edit="true"] 外壳文案元数据，以及业务 script/style/widget-config。外壳文案元数据只允许修改 data-title、data-goal 和最多三个 li 学习目标；不得向其中加入布局或样式结构。
...
- 连续曲线必须按变量完整定义域稳定采样并在首屏可辨认；教学进度若需逐步描画，使用 clipPath、mask 或 stroke-dasharray 揭示既有完整路径，禁止通过收缩采样定义域让默认值处退化为零长度曲线。
...
- 可见 DOM 文本禁止保留 `$...$` 或 `$$...$$` 定界符；所有静态公式必须使用 `<span data-katex="LaTeX源码">可读纯文本</span>` 等显式目标，并在初始化时遍历目标直接调用受 `window.katex` 守卫的 `katex.render`。纯文本 fallback 不得包含 `$` 定界符。
...
- 动态创建的 SVG/Canvas/DOM 节点必须先由 buildScene/createScene 完成赋值，再绑定该节点的事件并执行首次 render；禁止在 init/buildScene 调用前对未初始化节点执行 addEventListener、setAttribute 或其他 DOM 方法。不得用空值 early-return 掩盖初始化顺序错误后仍设置 runtime ready。
...
- 输出 HTML 必须控制在 {MODEL_HTML_TARGET_CHARS} 字符以内，绝对不要超过 {MODEL_HTML_HARD_LIMIT_CHARS} 字符；避免冗长注释、重复 CSS、内联大数据、base64、超长文案和重复 DOM，确保后续 HTML 修改阶段不会因上下文上限截断尾部脚本。
...
REPRESENTATION_PROMPT_MODULES = {
    ...
    "number_line": "数轴表征：数值、刻度、点位、区间与方向由同一尺度映射生成；缩放或参数变化后同步更新位置、标签和边界关系。",
    "tree_diagram": "树状图表征：父子层级、分支顺序、路径与当前焦点来自同一结构数据；展开、折叠和逐步揭示不得破坏引用关系。",
    "dynamic_model": "动态模型表征：对象状态、转换条件、时间或参数驱动与可见读数共用同一模型；明确模型边界并覆盖初始、边界和重置状态。",
...
- 编辑诊断、目标 selector 和函数名只是辅助证据，不是修改边界；必须阅读完整 HTML 独立确认根因。若实际根因位于相邻函数、状态推导、事件绑定或渲染路径，应修改真正根因及全部必要调用点。
- 用户输入描述的是期望结果，不保证准确指出实现位置。必须结合完整 HTML 自行判断真实意图；提到“控制面板、外壳、侧栏、布局、挤压”等词时，不得因此拒绝或机械修改外层结构，应优先检查主视觉尺寸/viewBox、槽位内部自适应、业务控件密度、标签与动画渲染链路，并实现用户可观察的目标。
- 必须保持用户未要求修改的教学内容、视觉层级、交互行为和功能一致；不得依据计划摘要、历史消息或其他旧上下文重新解释页面。
- 若用户明确要求“全部修改、整体重做、重新设计”，可以重做全部可编辑内容（外壳标题/目标文案、教学文案、主视觉、业务控件、状态、渲染、事件和动画运行时），此时不要套用“未要求部分保持不变”；但仍须保持用户明确指定的约束和核心 Widget 运行契约。
...
- 修复运行时错误时必须追溯初始化顺序和报错对象的创建位置；动态节点事件绑定只能发生在场景创建之后。不得仅增加空值 early-return、try/catch 或吞错逻辑来让报错消失，初始化成功后主视觉必须非空并且才能设置 runtime ready。
- DOM API 参数类型必须一致：传给 querySelector/querySelectorAll 的值必须是 CSS 字符串；若渲染辅助函数也允许调用方传入 Element，必须先按参数类型分流，禁止把 Element 隐式转成 `[object HTML*Element]` 选择器。
- 若页面包含 KaTeX，可见文本不得保留 `$...$`/`$$...$$`；静态公式使用 data-katex 显式目标并在初始化时直接调用受 window.katex 守卫的 katex.render，fallback 文本不得包含 `$`。
...
- 动画修改必须形成完整影响闭环：在内部依次核对输入事件 -> 业务状态 -> deriveView/等价推导 -> applyView/render -> AetherVizAnimationController/GSAP 时间源 -> reset/replay。用户要求的变化必须能从画面、状态或交互结果中观察到，禁止只改未被调用的函数、孤立变量、注释或不可达样式。
- 用户要求改变动画表现、运动轨迹、节奏、步骤或交互方式时，允许同时调整 SVG/Canvas 图元、时间线、状态模型和控制器接线；不要因为“最小修改”而只替换一个函数或一个数值。
...
- 修改后的完整 HTML 必须控制在 {MODEL_HTML_TARGET_CHARS} 字符以内，绝对不要超过 {MODEL_HTML_HARD_LIMIT_CHARS} 字符；如果原文件接近上限，应在不破坏功能的前提下精简重复样式、注释、静态文案和冗余 DOM，确保后续修改仍能完整放入上下文。
...
def build_interactive_generation_prompt(topic: str, plan: dict) -> str:
    ...
    if isinstance(scene_outline, dict):
        ...
        scene_outline = {
            key: value for key, value in scene_outline.items() if key not in {"widgetOutline", "widget_outline"}
        }
