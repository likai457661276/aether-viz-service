...
def create_chat_model(kind: str, *, response_schema: dict[str, Any] | None = None):
    ...
    if kind == "routing":
        routing_kwargs: dict[str, Any] = {
            ...
            "base_url": _blank_to_none(settings.openai_base_url),
            "temperature": 0.0,
            "max_tokens": max(settings.aetherviz_ir_router_max_tokens, 256),
            "timeout": max(settings.aetherviz_ir_router_timeout_seconds, 1),
            "max_retries": max(settings.aetherviz_ir_router_max_retries, 0),
            "extra_body": {"enable_thinking": False},
            "model_kwargs": {
                "response_format": (
                    {
        ...
        return ChatOpenAI(**routing_kwargs)
