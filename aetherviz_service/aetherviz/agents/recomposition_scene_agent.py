...
def _generate_ranked_scene_source(
    ...
    ranking = rank_geometry_ir_candidates(candidates, plan)
...
def _attempt_waypoint_completion(
    ...
    completed_ranking = rank_geometry_ir_candidates(
...
def _repair_scene_source(
    ...
    ranking = rank_geometry_ir_candidates([geometry_ir], plan, origins=["repair"])
    if not ranking["ok"]:
