...
AI互动实验通过 `/generate-aetherviz-spec` 根据教学主题生成完整独立 HTML。服务采用同端 SSE：`phase=plan` 流式生成并返回单页 `interactive` 计划，字段包括 `page_type`、`interactive_type`、`scene_outline`、`widget_outline`、`design_brief`、`interactive_spec`、`widget_actions`、`teaching_flow`、`controls`、`runtime` 等；前端确认后用 `phase=generate` 携带 `approved_plan` 生成自包含互动 HTML；教案修改用 `phase=revise` 返回新的 `plan_ready`；HTML 文件修改用 `phase=edit` 携带选中文件全文 `current_html` 并返回新的 `done.html` 分支。生成和编辑阶段按“模型 HTML -> 基础校验 -> 自动修复”输出，生产链路只硬拦截文档结构、安全边界和内联脚本语法问题。动态生成逻辑以 OpenMAIC interactive 为核心，生成物运行时使用原生 HTML/CSS/JS、Canvas/SVG、`requestAnimationFrame`，不使用 GSAP。
...
- 后端职责：负责 OpenMAIC widget 计划、HTML 生成、HTML 文件编辑、自动修复、基础结构安全校验和最终自包含 HTML 输出。
...
- 接口协作：前端按 `phase=plan -> plan_ready -> phase=generate -> done.html` 流程工作；`phase=revise` 只发送主题、修改意见和摘要型 `context`，用于修改教案；`phase=edit` 发送选中 HTML 文件全文 `current_html`、修改意见和摘要型 `context`，用于生成新的 HTML 分支，不覆盖旧 HTML。
...
- `react.py` 负责 SSE 事件生成、LLM 规划、互动 HTML 生成、HTML 文件编辑、自动修复和错误收敛。
...
- `validator.py` 负责 HTML 的基础文档结构、安全边界和内联脚本语法校验；OpenMAIC 契约、交互和可视化区域检查保留为开发期测试能力，不作为生产生成链路硬拦截。
- `edit_stream.py` 负责基于选中的 HTML 文件全文和用户修改意见生成新的 HTML 分支。
...
- 生产生成链路只做基础语法/安全校验，避免因质量门过严阻断可继续通过 chat 改进的 HTML。
