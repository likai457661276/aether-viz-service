...
- HTML **生成**与**编辑**分属 `generate/` 与 `edit/` 包，业务代码互不 import；共享的装配、校验、修复执行逻辑放在 `contracts/`（不含生成向交付 prompt）。
- 工作流层负责编排，不堆积底层解析、检查或修复实现；`workflow/` 仅承载计划相关流程。
...
- **Plan 生命周期**：`plan` / `revise_plan` / `approve_plan` 仅在生成 HTML **之前**有效；一旦发起生成，plan 不再作为聊天上下文被引用或修订，后续修改走 `edit_html`（以当前 HTML 为基线）。
- 生成提示词应保持通用、分层和可组合，优先围绕互动类型、学科类别、概念族、表征类型和教学模式组织；视觉/数值/布局/舞台等交付片段仅属于 `generate/prompts.py`。
- 编辑提示词以当前 HTML 为唯一事实基线，**不得**再注入上述生成向交付片段；`edit_html` 忽略请求中的 `plan_summary` 业务语义。编辑诊断须输出可机器检查的 `change_checks` / `preserve_checks`，由服务端做硬验收与失败证据重试。
- 不得为单个知识点增加专用完整提示词、静态 HTML 模板、关键词特例或生成分支。
- 新增分类必须能够覆盖一组可复用的教学主题，并通过测试说明其路由和降级行为。
- 知识画像是生成路由提示，不应被视为学科事实来源；低置信度或未命中时必须有通用降级路径。
- 计划归一化、生成提示、编辑提示、修复逻辑和运行时契约应保持语义一致，避免同一规则在多处独立写死；执行期安全/布局契约以 `contracts/` 代码为准，不以再灌生成说明书替代。
...
- 完整 HTML 生成遇到可重试的传输中断或不完整输出时，允许整次重新生成一次；重试仍未获得完整、可校验 HTML 时必须明确失败，禁止把残缺输出、占位页或低质量确定性页面作为降级结果交付。
...
- 生成后前端不得再将 plan 作为输入框上下文引用项，也不得再通过选中 plan 触发 `revise_plan`；`edit_html` 请求不应携带 `plan_summary`。
...
- 具有代表性的生成、修复或动画运行失败，可以从完整 LangSmith Trace 构建单步评估数据集，用于后续工作流回归。
- 数据集应按互动类型、表征类型和通用失败模式组织，不得为单个知识点增加专用模板、关键词分支或期望实现。
- Dataset 和 Evaluator 只允许作为仓库内或本地忽略目录中的文件存在，并且只允许在本地或离线回归流程中执行。
- 禁止通过 LangSmith CLI、SDK、API 或 UI 创建、上传、覆盖、绑定、删除或公开分享远程 Dataset/Evaluator；即使本地已配置 LangSmith 凭据也不得尝试。除非用户在具体任务中明确撤销本限制。
- 经用户明确要求查询的 LangSmith Trace 可以作为本地 Dataset 的输入；写入本地文件前必须移除密钥、个人信息、内部 URL 和其他敏感数据。
- 修改生成提示、修复流程、HTML 装配、动画运行时契约或相关校验器时，应使用已有代表性失败数据集进行回归。
- 浏览器行为评估和 LLM-as-Judge 保持在本地或离线流程，不进入生产同步请求链路。
- 本地 LLM Evaluator 默认使用按风险抽样、结果缓存和有界重试；仅对低分或分歧样本使用更高成本模型二审，不得默认全量调用高成本模型。
