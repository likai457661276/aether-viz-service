...
AI互动实验通过 `/generate-aetherviz-spec` 根据教学主题生成完整独立 HTML。服务采用同端双阶段 SSE：`phase=plan` 流式生成并返回单页 `interactive` 计划，字段包括 `page_type`、`interactive_type`、`interactive_spec`、`teaching_flow`、`controls`、`runtime` 等；前端确认后再用 `phase=generate` 携带 `approved_plan` 生成自包含互动 HTML。生成阶段按“模型 HTML -> 模型自动修复 -> OpenMAIC 稳定模板”三层输出，最终结果必须通过严格校验后才能返回。动态生成逻辑以 OpenMAIC interactive 为核心，生成物需包含 `script#widget-config` 和 iframe message action listener，运行时使用原生 HTML/CSS/JS、Canvas/SVG、`requestAnimationFrame`，不使用 GSAP。
...
- 后端职责：负责 OpenMAIC widget 计划、HTML 生成、自动修复、分型兜底、结构安全校验和最终自包含 HTML 输出。
...
- 接口协作：前端按 `phase=plan -> plan_ready -> phase=generate -> done.html` 流程工作；`phase=revise` 只发送主题、修改意见和摘要型 `context`，不得发送完整 HTML。
...
- `aetherviz.py` 负责 AI互动实验 HTTP 入参校验和响应封装，当前只保留 `/generate-aetherviz-spec`。
...
- `react.py` 负责 SSE 事件生成、双阶段 LLM 规划与互动 HTML fallback 编排、自动修复、OpenMAIC 模板兜底和错误收敛。
- `fallback_planner.py` 负责内置学科识别、互动类型识别、渲染栈规划、舞台布局与教学流程规划、规划提示词构建、规划 JSON 解析和默认规划兜底；不要依赖外部 skill。
- `fallback_validator.py` 负责互动 HTML 输出提取、代码围栏清理、截断内容闭合和基础长度检查。
- `openmaic_template.py` 负责在模型 HTML 和自动修复都不可用时，根据确认后的 plan 生成确定性 OpenMAIC 单页互动 HTML；该模板仍必须通过 `validator.py`，不得恢复旧静态页面或旧提示词。
- `validator.py` 负责 fallback HTML 的结构、安全、依赖、OpenMAIC 契约、交互和可视化区域严格校验。
- `theme.py` 负责从教学主题中提取生成主色。
...
- 计划对象继续以 `page_type: "interactive"` 为主，保留 `interactive_type` 兼容前端；可补充 `widget_type` 或 `widget_outline`，但不得破坏现有前端字段。
- `simulation`、`diagram`、`game` 使用独立 prompt、独立 widget-config 约束、分型校验和分型兜底模板。
- `openmaic_template.py` 使用分型兜底，不应让所有主题共用同一套主舞台图形。
- `validator.py` 执行主题语义与 `interactive_spec` 对主舞台元素的最小一致性检查。
- `metadata.source=openmaic_template` 应明确表示降级输出，供前端展示质量状态。
...
- 修改本地启动方式。
- 修改结构目录或关键约定。
- 修改 AI互动实验生成链路、主题色提取、fallback 路径或开发命令。
- 修改 Docker 启动方式。
