...
- `validator.py` 负责 HTML 的基础文档结构、安全边界、长度上限和内联脚本语法校验；OpenMAIC 契约、交互和可视化区域检查保留为开发期测试能力，不作为生产生成链路硬拦截。
