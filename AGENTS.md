...
AI互动实验通过 `/generate-aetherviz-spec` 根据教学主题生成完整独立 HTML。服务采用同端 SSE：`phase=plan` 流式生成并返回单页 `interactive` 计划，字段包括 `page_type`、`interactive_type`、`scene_outline`、`widget_outline`、`design_brief`、`interactive_spec`、`widget_actions`、`teaching_flow`、`controls`、`runtime` 等；前端确认后用 `phase=generate` 携带 `approved_plan` 生成自包含互动 HTML；HTML 文件修改用 `phase=edit` 携带选中文件全文 `current_html` 并返回新的 `done.html` 分支。生成和编辑阶段按“模型 HTML -> 基础校验 -> 自动修复”输出，生产链路硬拦截文档结构、安全边界、内联脚本语法和 HTML 长度上限。动态生成逻辑以 OpenMAIC interactive 为核心，生成物运行时使用原生 HTML/CSS/JS、Canvas/SVG、`requestAnimationFrame`，并可使用白名单 GSAP core CDN 管理教学分镜。
...
- 接口协作：前端按 `phase=plan -> plan_ready -> phase=generate -> done.html` 流程工作；后续修改仅走 `phase=edit`，发送选中 HTML 文件全文 `current_html`、修改意见和摘要型 `context`，用于生成新的 HTML 分支，不覆盖旧 HTML。
